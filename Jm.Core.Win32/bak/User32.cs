...
namespace Jm.Core.Win32Api
{
    ...
    {
        /// <summary>
        /// 宏将图标或光标添加到图像列表中。ImageList_AddIcon调用ImageList_ReplaceIcon功能。
        /// </summary>
        /// <param name="himl">处理图像列表</param>
        /// <param name="hIcon">处理图标或光标</param>
        /// <returns></returns>
        [DllImport("COMCTL32")] public static extern int ImageList_AddIcon(HANDLE himl, HANDLE hIcon);
        /// <summary>
        /// 创建一个新的图像列表。
        /// </summary>
        /// <param name="MinCx">指定每个图像的宽度（以像素为单位）。</param>
        /// <param name="MinCy">指定每个图像的宽度（以像素为单位）。</param>
        /// <param name="flags">
        ...
        /// </param>
        /// <param name="cInitial">图像列表最初包含的图像数。</param>
        /// <param name="cGrow">当系统需要调整列表大小以为新图像腾出空间时，图像列表可以增长的图像数量。此参数表示调整大小的图像列表可以包含的新图像的数量。</param>
        /// <returns>如果函数成功，则返回值是图像列表的句柄。如果函数失败，返回值为NULL。</returns>
        [DllImport("COMCTL32")] public static extern int ImageList_Create(int MinCx, int MinCy, int flags, int cInitial, int cGrow);
        /// <summary>
        /// 在指定的设备上下文中绘制一个图像列表项
        /// </summary>
        /// <param name="hIMAGELIST">处理图像列表。</param>
        /// <param name="ImgIndex">指定要绘制的图像的索引。</param>
        /// <param name="hdcDest">处理目标设备上下文。</param>
        /// <param name="xDest">指定在指定的设备上下文中绘制的x坐标。</param>
        /// <param name="yDest">指定在指定的设备上下文中绘制的y坐标。</param>
        /// <param name="lStyle">指定绘图样式的标志。此参数可以是以下值中的一个或多个：
        /// 值 含义
        /// ILD_BLEND25,
        /// ILD_FOCUS 绘制图像，混合25％与系统高亮颜色。如果图像列表不包含掩码，则此值不起作用。
        /// ILD_BLEND50,
        /// ILD_SELECTED,
        /// ILD_BLEND 绘制图像，混合50％的系统高亮颜色。如果图像列表不包含掩码，则此值不起作用。
        /// ILD_MASK 画面具。
        ...
        /// <summary>
        /// 根据图像列表中的图像和掩码创建一个图标或光标。
        /// </summary>
        /// <param name="hIMAGELIST">处理图像列表。</param>
        /// <param name="ImgIndex">图像索引</param>
        /// <param name="hbmMask"></param>
        /// <returns>如果函数成功，则返回值是图标或光标的句柄。如果函数失败，返回值为NULL。</returns>
